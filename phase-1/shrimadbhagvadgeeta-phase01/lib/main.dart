...
Future<void> main() async {
  ...
  // Replace the placeholder values with your project's URL and anon key from:
  // https://supabase.com/dashboard → Project Settings → API
  await Supabase.initialize(
    url: 'https://yczxjrzhmemiaodwjctz.supabase.co',       // e.g. https://xyzcompany.supabase.co
  ...
  // Order doesn't matter — type IDs must match HiveTypeIds constants exactly.
  Hive
  ...
  // ── Step 4: Launch app with Hive box overrides ───────────────────────────
  runApp(
    ProviderScope(
      overrides: [
...
class GeetaApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // Watch syncTriggerProvider here so it persists for the full app lifetime.
    // This is required for it to react to auth state changes on login.
    ...
    return MaterialApp.router(
      ...
      // TODO (Step 7): themeMode: ref.watch(settingsProvider).themeMode
      routerConfig: router,
    );
  }
}
