...
// ─────────────────────────────────────────────────────────────────────────────
// LAYER 0 — External SDKs (Supabase, Dio)
// ─────────────────────────────────────────────────────────────────────────────

/// The initialized [SupabaseClient]. Supabase.initialize() must be called
/// before this provider is read (done in main.dart before runApp).
...
// ─────────────────────────────────────────────────────────────────────────────
// LAYER 3 — Repositories (exposed as domain interfaces)
// ─────────────────────────────────────────────────────────────────────────────

// ── Auth ──────────────────────────────────────────────────────────────────────

/// Keeps [AuthRepository] reactive to Supabase client changes.
...
// ── Bookmarks — sync-aware (userId injected at construction) ──────────────────

/// Rebuilds whenever [currentUserIdProvider] changes (login/logout).
/// When userId is null (unauthenticated) sync calls are skipped silently.
///
/// NOTE: [currentUserIdProvider] is defined in auth_state_provider.dart
/// to avoid circular imports. This provider must be imported from there.
...
// ── Collections — sync-aware ──────────────────────────────────────────────────
final collectionRepositoryProvider = Provider<CollectionRepository>((ref) {
  return CollectionRepositoryImpl(ref.watch(collectionLocalDsProvider));
});

/// Derived from [authStateProvider] -- null when unauthenticated.
/// Used by [bookmarkRepositoryProvider] to decide whether to sync.
